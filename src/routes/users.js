...
router.get('/search', requireAuth, async (req, res) => {
  try {
    ...
    const usersResult = await db.query(`SELECT u.id, u.farmflow_id, u.user_type, u.full_name, u.phone_number, u.country, u.state, u.status, u.registered_at FROM users u ${where} ORDER BY u.registered_at DESC LIMIT $${i} OFFSET $${i+1}`, params);
...
router.get('/:farmflowId', requireAuth, async (req, res) => {
  try {
    ...
    const userResult = await db.query(`SELECT u.*, a.username as registered_by_admin FROM users u LEFT JOIN admins a ON a.id = u.registered_by_admin_id WHERE u.farmflow_id = $1`, [farmflowId.toUpperCase()]);
...
router.patch('/:farmflowId/status', requireAuth, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE users SET status = $1, suspension_reason = $2, updated_at = NOW() WHERE farmflow_id = $3`, [status, reason||null, farmflowId]);
    await logActivity(req.admin.id, `STATUS_${status.toUpperCase()}`, null, farmflowId, reason, req.ip);
